...
- **Missing-category fallback:** If an upstream category error leaves that category out of the current cache, the historical importer falls back to product specs from the category's latest stored snapshot. On split repository/web-root deployments, run maintenance scripts with `IPPGI_WP_ROOT=/home/html/www/ippgi`; do not copy them into the public web root.
