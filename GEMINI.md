...
- **PHP 8.1 compatibility note for SWPM:** `simple-membership/classes/class.swpm-utils-misc.php::get_countries_dropdown()` has a required local compatibility patch: `$country = is_string( $country ) ? $country : '';`. Official SWPM 4.7.9 still lacks it, while production member rows have been observed with SQL `NULL` country values. Upgrade SWPM in Git first, reapply this patch, then deploy; do not rely on an in-place WordPress-admin update. The expected checksum exception is limited to this file.
- **Production plugin deployment:** The production Git checkout (`/home/wlg2008g/deploy/ippgi-src`) and web root (`/home/html/www/ippgi`) are separate. Deploy themes/plugins one directory at a time. Never run `rsync --delete` over the whole `wp-content/plugins/` tree because production can contain untracked operational plugins such as Wordfence; limit source and destination to the same specific plugin directory.
- **Production plugin permissions:** Deployed plugin code is `root:www-data`, directories `0755`, files `0644`; `www-data` must not be able to modify code. Only explicit runtime directories such as `uploads` and `wflogs` are writable. Exclude `.DS_Store` and `log-*.txt` from deployment.
