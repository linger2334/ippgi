...
- 定时任务关键流程：
  - 00:00：先刷新最新汇率，再按最新汇率重算现有价格列表缓存和已缓存单规格详情中的美元价格（人民币价格保持不变），然后保存价格/汇率快照。
  - 09:00-17:00：按小时刷新价格。采用“增量更新”策略：保留现有价格列表缓存，逐个分类获取新数据；若获取失败则保留该分类的旧数据，并强制按最新汇率重新计算美元价，以确保全站汇率一致性及应对 API 不稳定。
